"""
实验二 Part3：DDPO 训练 (r_in 内生奖励 — tanh 锚定映射).

与 Part2 的核心区别:
    - Part2: r_in = zscore(total_log_p_base) 组内归一化 → 丢失绝对质量信号
    - Part3: r_in = tanh((total_log_p_base - center) / temperature)
            以 base model 典型值为锚点, 同时保留绝对量级和组内区分度

设计动机:
    z-score 归一化把绝对质量信号抹掉了:
    - 模型偏离 base 后, 所有链的 raw log_p 一起跌
    - z-score 还在选 "矮子里拔将军", PPO 不知道整体质量在下降

    tanh 映射兼顾两者:
    - center (~300k) = base model 锚点, raw 偏离中心映射到 [-1, 1]
    - temperature (~2000) 控制组内区分度, 1σ(~890) → r_in 差 ~0.42
    - 值域 [-1, 1] 天然有界, 数值稳定

训练流程 (per epoch):
    1. 随机选取 2 个 prompt, 各 6 条链 = 12 chains
    2. 用 frozen base model 计算 total_log_p_base
    3. r_in = tanh((total_log_p_base - center) / temperature)
    4. PPO: per-step ratio = exp(log_p_new - log_p_old), clip only
    5. LoRA fine-tuning, 4 gradient updates per epoch (mini_batch=3)

WandB 输出:
    - total_log_p 分布 (raw + r_in)
    - PPO loss, ratio mean
    - 每 N epoch 采样图像
"""

# 运行方式 (在 SRDM-DDPO/ 目录下):
#     python experiments/exp2/exp2_part3.py --config config/exp2_part3_config.py
...
def run_training(cfg):
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        # ---- 7c. r_in = tanh((total_log_p_base - center) / temperature) ----
        # tanh 映射: 以 base model 典型值 (~300k) 为锚点, temperature 控制组内区分度
        #   center=300k, temp=2000 → 1σ(~890) ≈ r_in 差 0.42, 值域 [-1, 1]
        #   接近 base → r_in ≈ 0; 偏离 → r_in → -1; 质量好 → r_in → +1
        total_lps = torch.tensor([d["total_lp_base"] for d in all_chain_data], device=device)
        r_in = torch.tanh((total_lps - cfg.r_in_center) / cfg.r_in_temperature)
