"""实验三 Part 1: VLM 结构提取验证 + 速度 Benchmark.

Thin experiment script — core logic lives in srdm_pytorch_exp/:
  - vlm_client.py:         VLMClient, VLMVariant, benchmark_variants_batch,
                           draw_structure_annotations, compare_structures
  - structure_features.py:  phi_* computation (not used in Part 1)
  - reward_ssr.py:          r_SSR computation (not used in Part 1)

实验设计:
  - 5 个结构描述 prompt × 6 条 SDE 链 = 30 张图像
  - 5 种 VLM 提取 variant (baseline_512, compress_256, grayscale_512,
    center_crop, no_thinking)
  - 循环: variant 优先 → 每个 variant 下 5 prompt × 6 图并行发给 VLM
  - baseline_512 始终第一个跑，作为质量参照

Usage:
    python experiments/exp3/exp3_part1.py --config config/exp3_part1_config.py
"""
...
VARIANTS = [
    ...
    VLMVariant("compress_256",   "Compress 256px",            "压缩大小",        256),
    VLMVariant("grayscale_512",  "Grayscale 512px",           "灰度化/去彩色",   512, grayscale=True),
    VLMVariant("center_crop",    "Center Crop 384px",         "裁剪有效ROI",     384, center_crop_ratio=0.7),
    VLMVariant("no_thinking",    "No Thinking",               "减少思考轮次",    512, disable_thinking=True),
]

# ============================================================
# Prompts (结构描述类，含明确的空间/数量关系)
# ============================================================
...
PROMPTS = [
    ...
    "Three yellow cylinders stacked vertically with a green cone on top. "
    "Minimalist style, clean background.",

    "A large wooden table in the center, four white chairs around it, "
    "one chair on each side of the table. Photorealistic interior.",

    "A silver car on the left, a red bicycle on the right, "
    "a traffic light between them. Bright sunny outdoor scene.",

    "Five orange spheres arranged in a circle with a single blue cube "
    "at the very center. Top-down bird's eye view.",
]
...
def _load_config(config_path: str):
    """Load config from a .py file path (compatible with exp2 pattern)."""
...
# ============================================================
# Core: generate 6 SDE chains for a single prompt
# ============================================================

def generate_chains(
    pipeline, prompt: str, device: str, config,
) -> tuple:
    """Generate 6 SDE chains for one prompt.

    Returns:
        pil_images: List[Image] (6 images)
        log_p_base_totals: List[float] (6 values)
    """
...
# ============================================================
# Core: run one variant across all prompts (parallel per prompt)
# ============================================================

def run_variant_across_prompts(
    vlm_client: VLMClient,
    variant: VLMVariant,
    all_prompt_data: list,  # list of dicts with keys: prompt, schema, pil_images
    stagger_delay: float = 2.0,
) -> list:
    """Run one VLM variant across all prompts, 6 images parallel per prompt.

    Args:
        vlm_client: VLMClient instance.
        variant: VLMVariant to run.
        all_prompt_data: list of per-prompt dicts.
        stagger_delay: delay between launching threads (avoids 429).

    Returns:
        all_prompt_data updated in-place with variant results added.
    ...
    for p_idx, pdata in enumerate(all_prompt_data):
        ...
        # ---- Parallel VLM calls for all 6 images ----
        results: list = [None] * M  # type: ignore
...
def main(config_path: str):
    ...
    print(f"{'='*70}")
    print(f"  Experiment 3 Part 1: VLM Structure Extraction + Speed Benchmark")
    print(f"{'='*70}")
    ...
    print(f"  SDE:       a={config.a}, steps={config.num_inference_steps}")
    ...
    # ================================================================
    # 3. Generate images for all 5 prompts (6 chains each)
    # ================================================================
    print(f"\n{'='*70}")
    print(f"  3. Generating {len(PROMPTS)} prompts × "
          f"{config.num_chains_per_prompt} chains = "
          f"{len(PROMPTS) * config.num_chains_per_prompt} images")
    print(f"{'='*70}")
    ...
    for p_idx, prompt in enumerate(PROMPTS):
        ...
        if config.visualize:
            ...
            wandb.log({f"exp3_part1/p{p_idx}/overview_raw": wandb.Image(
    ...
    # ================================================================
    # 4. VLM Extraction — variant-first loop, 6 images parallel
    #    Baseline runs first, others compared against it
    # ================================================================
    print(f"\n{'='*70}")
    print(f"  4. VLM Extraction — {len(VARIANTS)} variants × "
          f"{len(PROMPTS)} prompts × {config.num_chains_per_prompt} chains")
    print(f"     Loop: variant → parallel 6 images per prompt")
    print(f"{'='*70}")

    variant_order = sorted(VARIANTS, key=lambda v: (0 if v.is_baseline else 1))
    baseline_structures = None  # per-prompt per-chain baseline results

    for v_idx, variant in enumerate(variant_order):
        t0_variant = time.time()

        # Run this variant across all prompts
        run_variant_across_prompts(vlm, variant, all_prompt_data)

        elapsed_v = time.time() - t0_variant
        print(f"  [Variant {variant.key} total: {elapsed_v:.1f}s]")

        # ---- Compute quality vs baseline (skip for baseline itself) ----
        if not variant.is_baseline:
            baseline_key = "results_baseline_512"
        ...
        # ---- Wandb logging for this variant (all prompts, all chains) ----
        for p_idx, pdata in enumerate(all_prompt_data):
            pil_images = pdata["pil_images"]
            vr_key = f"results_{variant.key}"
            results = pdata.get(vr_key, [])
            prompt = pdata["prompt"]

            # Per-chain annotated images + timing
            for c_idx, vr in enumerate(results):
                tag = f"exp3_part1/p{p_idx}/c{c_idx}/variants/{variant.key}"
                ...
                if config.visualize:
                    ...
                    caption = (f"{variant.label} | {variant.strategy} | "
                               f"{vr['elapsed']:.1f}s | {obj_summary}")
                    if err:
                        caption += f" | ERR: {err}"
                    wandb.log({
            ...
            # Per-prompt × variant summary table
            tag = f"exp3_part1/p{p_idx}/variant_comparison"
            comp_rows = []
            for c_idx, vr in enumerate(results):
                q = vr.get("quality_vs_baseline")
                comp_rows.append([
    ...
    # ================================================================
    # 5. Cross-variant summary (aggregated across all prompts)
    # ================================================================
    print(f"\n{'='*70}")
    ...
    print(f"\n  {'Variant':<24} {'Avg Time':>10} {'Speedup':>10}  "
    ...
    for variant in VARIANTS:
        ...
        print(f"  {variant.label:<24} {t:>8.2f}s  {speedup:>8.2f}x  "
              f"{ca_str:>10}  {bi_str:>10}")

    # Wandb summary charts
    timing_summary = [[v.label, avg_times[v.key]] for v in VARIANTS if avg_times[v.key] > 0]
    wandb.log({"exp3_part1/summary/variant_avg_time": wandb.plot.bar(
        wandb.Table(columns=["variant", "avg_time_sec"], data=timing_summary),
        "variant", "avg_time_sec",
        title="Average VLM Extraction Time per Variant (all prompts)",
    )})
    ...
    wandb.log({"exp3_part1/summary/speedup_quality_table": wandb.Table(
    ...
    print(f"  Total VLM calls: {len(PROMPTS) * config.num_chains_per_prompt * len(VARIANTS)}")
    print(f"{'='*70}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, required=True,
                        help="Path to config .py file, e.g. config/exp3_part1_config.py")
