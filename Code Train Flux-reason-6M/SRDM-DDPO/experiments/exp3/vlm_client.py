"""
VLM Client — Multi-backend support (Doubao + Qwen) + speed benchmarking.

两阶段结构提取 (v2):
    Phase 1: spaCy 提取名词 → schema JSON (本地, <0.01s, 零费用)
...
用法:
    client = VLMClient(backend="qwen_local", model="/path/to/Qwen2.5-VL-7B-Instruct")
    schema = client.extract_schema("three cats and two dogs")
    structure = client.extract_structure(pil_image, schema)

    # Speed benchmark
    from srdm_pytorch_exp.vlm_client import VLMVariant, benchmark_variants
...
_STRUCTURE_PROMPT_TEMPLATE = """Detect each object from the list below in this image. For each object, record:
...
IMPORTANT: Do NOT count large continuous background regions (walls, floors, sky, ground, water, etc.) as object instances. Only detect the specific foreground objects listed below that are clearly visible and distinguishable. If an object blends into the background or is ambiguous, set count=0.
...
class VLMClient:
    ...
    def extract_structure(self, image: Image.Image, schema: dict, original_prompt: str = "",
        ...
        user_text = _STRUCTURE_PROMPT_TEMPLATE.format(OBJECT_LABELS=labels_str)
        messages = [
            {
                "role": "user",
                "content": [
