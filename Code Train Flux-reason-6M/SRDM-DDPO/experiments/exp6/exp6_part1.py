"""
exp6 Part 1: r_in + r_SSR DDPO 训练 — 纯内生奖励组合.

奖励规则:
    r_total = 0.5 * r_in + 1.0 * r_SSR
    - r_in = 组内 z-score (base model log_prob)
    - r_SSR = -d_combined, 其中 φ* = M 条链均匀平均

与 exp5 的关键区别:
    - 无 GT 监督 (无 r_gt)
    - φ* = 均匀平均 (uniform_weights=True)
    - 无 TrainingAlerter
    - 20k 提示词, 300 epochs, lr=5e-5

用法:
    python experiments/exp6/exp6_part1.py --config config/exp6_part1_config.py
    python experiments/exp6/exp6_part1.py --config config/exp6_part1_config.py --resume 20
"""
...
from experiments.exp6.reward_exp6 import compute_reward_exp6
...
def _run_model_epoch(state, selected_prompts, prompt_encodings, prompt_gt_map, epoch, cfg,
    ...
    """Sample (all) → VLM (parallel) → r_in + r_SSR Reward + PPO (per prompt).

    Phase 1: 采样所有 prompt 的 M 条 chain，后台提交 VLM。
    Phase 2: 逐 prompt 等 VLM → r_in z-score → r_SSR → PPO update。
    """
    ...
    try:
        ...
        # ---- Phase 2: Per-prompt VLM → r_in + r_SSR → PPO ----
        vlm_elapsed = 0.0
        ...
        for p_idx, future, p_chains in vlm_futures:
            ...
            if len(valid_chains) >= 2:
                r_in, r_ssr, rewards, ssr_debug = compute_reward_exp6(valid_chains, cfg)
                ...
                # Bad chains: min reward from valid
    ...
    # Extract d_combined from debug info
    d_combined_vals = []
        ...
        for p_idx in range(N):
            ...
            for chain in p_chains:
                ...
                try:
                    ...
                    caption = f"[{model_key}] e{epoch} p{p_idx} {label} | {reward_str}"
...
def run_training(cfg, resume_epoch=0):
    ...
    print(f"VLM: {cfg.vlm_backend} / {cfg.vlm_model} (thinking={'on' if not cfg.vlm_disable_thinking else 'off'}) | "
    ...
    print(f"Reward: {cfg.r_in_weight}*r_in + {cfg.r_ssr_weight}*r_SSR (φ* = uniform avg)")
    ...
    print(f"\n{'='*60}\nexp6 完成! model={model_key}, reward={cfg.reward_mode}\n{'='*60}")
    wandb.finish()
